<template>
  <svg
    ...
    :class="animated ? 'group-hover:scale-110' : ''"
  >
    <!-- Main Icon Shape -->
    <path
...
const primaryColor = computed(() => {
  if (props.gradient) return 'url(#logoGradient)'
  if (props.color === 'primary') return 'var(--ui-primary)'
  if (props.color === 'white') return '#ffffff'
  return props.color
})

const gradientStart = computed(() => {
  return props.color === 'primary' ? '#3b82f6' : '#8b5cf6'
})

const gradientEnd = computed(() => {
  return props.color === 'primary' ? '#6366f1' : '#ec489a'
})
</script>